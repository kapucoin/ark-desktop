;(function () {
  ...
  function LedgerService ($q, $http, $timeout, storageService, networkService) {
    ...
    function getBip44Accounts (slip44) {
      ...
      async.whilst(
        ...
        (next) => {
          ...
          if (result.address) {
            result.address = kapujs.crypto.getAddress(result.publicKey)
    ...
    function recoverBip44Accounts (backupLedgerPassphrase, slip44) {
      const hdnode = kapujs.HDNode.fromSeedHex(bip39.mnemonicToSeedHex(backupLedgerPassphrase))
    ...
    function signTransaction (path, transaction) {
      return new Promise((resolve, reject) => {
        ...
        ipcRenderer.send('ledger', {
          ...
          data: kapujs.crypto.getBytes(transaction, true, true).toString('hex'),
          path
        })
      })
    }
